export const cedarNilesData = {
        name: 'Cedar Niles',
        urlParam: 'cedar-niles',
        rank: 1,
        featuredImage: '/mtn-bike-kc/cedar-niles/cedar-niles-car.JPEG',
        stats: {
            distanceMiles: 7.5,
            numberOfTrails: 5,
            flowTrails: 0,
            downHillTrails: 1,
            XCTrails: 4,
            areaDificulty: 1,
            traffic: 'low',
            currentStatus: 'Open'
        },
        mapStartingLat: 38.906144,
        mapStartingLon: -94.879008,
        mapZoom: 14,
        gpxTracks: [
            {name: 'Columbine Connector', difficulty: 'easy', textColor: 'text-purple-600', color: '#9810fa', url: '/mtn-bike-kc/cedar-niles/gpx-files/columbine-connector.gpx', active: 1, oneWay: false, description: '', features: [], mustRide: false, rating: 3},
            {name: 'Columbine Loop', difficulty: 'easy', textColor: 'text-orange-600', color: '#f54a00', url: '/mtn-bike-kc/cedar-niles/gpx-files/columbine-loop.gpx', active: 1, oneWay: false, description: '', features: [], mustRide: false, rating: 3},
            {name: 'Columbine Bypass', difficulty: 'easy', textColor: 'text-blue-600', color: '#155dfc', url: '/mtn-bike-kc/cedar-niles/gpx-files/columbine-bypass.gpx', active: 1, oneWay: false, description: '', features: [], mustRide: false, rating: 3},
            {name: 'Billy Goat', difficulty: 'easy', textColor: 'text-green-600', color: '#00a63e', url: '/mtn-bike-kc/cedar-niles/gpx-files/billy-goat.gpx', active: 1, oneWay: false, description: '', features: [], mustRide: false, rating: 3},
            {name: 'Scape Goat', difficulty: 'easy', textColor: 'text-yellow-400', color: '#fcc800', url: '/mtn-bike-kc/cedar-niles/gpx-files/scape-goat.gpx', active: 1, oneWay: false, description: '', features: [], mustRide: false, rating: 3},
            {name: 'Technical Loop', difficulty: 'medium', textColor: 'text-red-600', color: '#fb2c36', url: '/mtn-bike-kc/cedar-niles/gpx-files/technical-loop.gpx', active: 1, oneWay: false, description: '', features: [], mustRide: false, rating: 3},
            {name: 'The Annex', difficulty: 'easy', textColor: 'text-purple-600', color: '#9810fa', url: '/mtn-bike-kc/cedar-niles/gpx-files/the-annex.gpx', active: 1, oneWay: false, description: '', features: [], mustRide: false, rating: 3},
            {name: 'Paved Path', difficulty: 'easy', textColor: 'text-gray-600', color: '#4a5565', url: '/mtn-bike-kc/cedar-niles/gpx-files/paved-path.gpx', active: 1, oneWay: false, description: '', features: [], mustRide: false, rating: 3},
        ],
        photos: [
            { photoUrl: "/mtn-bike-kc/cedar-niles/billy-goat-south-trail-head.JPEG", title: 'Billy Goat Trail Marker', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/chossy.JPEG", title: 'About As Chossy As It Gets', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/colorado-esc.JPEG", title: 'Through the Pines', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/columbine-connecter.JPEG", title: 'Columbine Connector Terrain', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/columbine-connecter-car.JPEG", title: 'The Iconic Car', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/columbine-connecter-trail-head.JPEG", title: 'Bypass East Trailhead', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/ledges.JPEG", title: 'Harder Terrain',show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/map.JPEG", title: 'System Map', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/overlook.JPEG", title: 'The Overlook', subtitle: "The picture dosn't do it justice", show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/trail-2.JPEG", title: 'Harder Terrain', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/trail-3.JPEG", title: 'Rock Section', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/trail-head-sign.JPEG", title: 'Scap Goat East Trail Head', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/trail.JPEG", title: 'Tyipical Terrain', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/the-annex.JPEG", title: 'The Annex Trailhead', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/dirt-jump.JPEG", title: 'Annex Dirt Jump', show: false},
            { photoUrl: "/mtn-bike-kc/cedar-niles/wood-drop.JPEG", title: 'The Annex Drop', subtitle: 'One of my favorite features in Kansas City', show: false},

            // "/mtn-bike-kc/cedar-niles/cedar-niles-car.JPEG",
            // "/mtn-bike-kc/cedar-niles/big-jump.JPEG",
            // "/mtn-bike-kc/cedar-niles/small-jump.JPEG",
            // "/mtn-bike-kc/cedar-niles/billy-goat-north-trail-head.JPEG",
            // "/mtn-bike-kc/cedar-niles/4-corrners.JPEG",
            // "/mtn-bike-kc/cedar-niles/typical-marker.JPEG",
            // "/mtn-bike-kc/cedar-niles/typical-trail.JPEG",
        ],
        accessInfo: [
            {
                trailheadName: 'Clare Road Trailhead',
                gMapLinkUrl: 'https://maps.app.goo.gl/hNYoWBXYHtPLdLH97',
                gMapEmbedUrl: 'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d8963.888413691491!2d-94.8790833808989!3d38.90040089327876!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x87c09717e637c3c3%3A0x326b4c9e2476ee0d!2sCedar%20Niles%20Park%20Clare%20Road%20Trailhead!5e0!3m2!1sen!2sus!4v1724972397538!5m2!1sen!2sus',
                trailheadDiscription: 'This is the main access point for the trails. From here you can jump right onto the Columbine Connector trail that will take you out to Billy Goat and the Columbine Loop.'
            },
            {
                trailheadName: '119th Street Trailhead',
                gMapLinkUrl: 'https://maps.app.goo.gl/KySzsCW7vRfB2vyd9',
                gMapEmbedUrl: 'https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d12419.436404190137!2d-94.88314610747071!3d38.90433725660361!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x87c0977bb9da0e6d%3A0x11414b7fb807be8c!2sCedar%20Niles%20Park%20119th%20St%20Trailhead!5e0!3m2!1sen!2sus!4v1724975254860!5m2!1sen!2sus',
                trailheadDiscription: 'I would call this secondary access. From here you will have to head down the paved path across the bridge and twords the woods. You will see the entrance to Scape Goat. Ride up Scape Goat to access the rest of the trail system.'
            }
        ],
        faqHighlights: [
            {
                title: 'Columbine Bypass',
                text: 'is a nice jump trail located in the middle of the trail system. Great place for laps!'
            },
            {
                title: 'Columbine Loop',
                text: 'is best riden counter clockwise'
            },
            {
                title: 'Technical Loop',
                text: 'is as chossy as this trail system gets. I prefer riding this trail counter clockwise.'
            }
        ],
        faqFoodDrink: [
            {
                title: 'Best post ride beer?',
                text: 'Red Crow Brewing is the hands down winner. I would go with an Isabelle & a Spicy Honey Mustard Chicken sandwich'
            },
            {
                title: 'Lunch Time?',
                text: "Bonito Michoacán is a grocery story that has a small cafe that serves authentic mexican food!"
            },
            {
                title: "BB's Grill",
                text: "BB's is better than I would have thought. Good beer selection and the Jalapeno Popper Chicken Sandwich is to die for."
            }
        ],
    }
