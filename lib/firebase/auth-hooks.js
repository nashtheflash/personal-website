...
export function useAggressiveAuth() {
  ...
  useEffect(() => {
    ...
    // Only clear assumed user if Firebase explicitly says no auth
    if (user === null && !loading && !isAuthenticated && authError) {
      setAssumedUser(null)
    }
  ...
  }, [user, loading, isAuthenticated, authError, assumedUser])
