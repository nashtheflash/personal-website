...
import { collection, addDoc, getDoc, getDocs, doc, setDoc, updateDoc, increment, Timestamp } from "firebase/firestore";

// GET DATA
export async function getTenantContent(tenantId) {
    const content = [];

    const snapshot = await adminDb.collection('content').where('tenant', 'array-contains', tenantId).get();
...
export async function addContentDocument({
    ...
    try {
        ...
        if(contentId) {
            ...
            const contentRef = collection(db, 'email_queue');
